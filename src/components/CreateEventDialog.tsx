...
interface CreateEventDialogProps {
  ...
  onEventCreated?: (prePopulatedContent: string, eventType: 'event', eventData: NostrEvent) => void;
  communityId?: string;
  channelId?: string;
}
...
export function CreateEventDialog({ open, onOpenChange, onEventCreated, communityId }: CreateEventDialogProps) {
  ...
  // Auto-fill community banner/image if available
  useEffect(() => {
    if (community && !formData.imageUrl) {
      setFormData(prev => ({
        ...prev,
        imageUrl: community.banner || community.image || ''
      }));
    }
  }, [community, formData.imageUrl]);
      ...
      createEvent({
        ...
        onSuccess: (calendarEvent) => {
          ...
          const nevent = nip19.neventEncode({
            ...
            author: calendarEvent.pubkey,
            relays: []
          });

          // Pre-populate message content with event reference (no prefix)
          const prePopulatedContent = `nostr:${nevent}`;

          // Call the callback with the pre-populated content
          if (onEventCreated) {
            onEventCreated(prePopulatedContent, 'event', calendarEvent);
          }
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto scrollbar-thin">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-2">
            ...
            {community && (community.banner || community.image) && (
              <p className="text-sm text-muted-foreground">
                Community banner/image will be used automatically if no image is provided
              </p>
            )}
          </div>
