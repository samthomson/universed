...
export function ZapDialog({ target, children, className }: ZapDialogProps) {
  ...
  const { mutateAsync: _createEvent } = useNostrPublish();
  ...
  useEffect(() => {
    if (target) {
      setComment('Zapped with Universes!');
    }
  }, [target]);
  ...
  const zap = async (zapAmount: number, zapComment: string) => {
    ...
    try {
      if (!author?.metadata || !author?.event) {
        throw new Error('Author not found');
      }

      const { lud06, lud16 } = author.metadata;
      if (!lud06 && !lud16) {
        throw new Error('Lightning address not found');
      }

      const zapEndpoint = await nip57.getZapEndpoint(author.event);
      if (!zapEndpoint) {
      ...
      const res = await fetch(`${zapEndpoint}?amount=${amountMillisats}&nostr=${encodeURI(JSON.stringify(signedZapRequest))}`);
      ...
      if (currentNWCConnection && currentNWCConnection.connectionString && currentNWCConnection.isConnected) {
        try {
          await nwcSendPayment(currentNWCConnection, newInvoice);
          setIsZapping(false);
          setInvoice(null);
          toast({
