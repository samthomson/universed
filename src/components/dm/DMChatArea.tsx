...
export function DMChatArea({ conversationId, onNavigateToDMs, onBack }: DMChatAreaProps) {
  ...
  return (
    <div className="flex flex-col h-full">
      {/* Header */}
      <div className={`${isMobile ? 'h-14' : 'h-16'} border-b border-gray-600 flex items-center justify-between px-4 bg-gray-800`}>
        <div className="flex items-center space-x-3">
          ...
          {isMobile && onBack && (
            <Button
              ...
              size="icon"
              onClick={onBack}
              className="w-8 h-8 text-gray-300 hover:text-white mobile-touch"
            >
              <ArrowLeft className="h-5 w-5" />
            </Button>
          )}

          <Avatar className={isMobile ? "w-8 h-8" : "w-8 h-8"}>
