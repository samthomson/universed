...
export function CommunitiesDebug() {
  ...
  const { communities: communitiesData, isLoading, loadingPhase, getDebugInfo } = communities;
...
function getCommunityName(definition: any): string | null {
  try {
...
function getCommunityDescription(definition: any): string | null {
  try {
    const content = JSON.parse(definition.content);
...
function getChannelName(definition: any): string | null {
  try {
    const content = JSON.parse(definition.content);
    return content.name || null;
  } catch {
...
function getChannelDescription(definition: any): string | null {
  try {
    const content = JSON.parse(definition.content);
    return content.about || content.description || null;
  } catch {
...
function getApprovedMembersCount(membersList: any): number {
  try {
...
function getApprovedMembersList(membersList: any): string[] {
  try {
    const content = JSON.parse(membersList.content);
...
function getPermissionsInfo(permissions: any): any {
  try {
